...
class CalendarHeader extends Component {
  ...
  renderHeader = () => {
    ...
    const dateString = month.toString(monthFormat);

    return (
      <Fragment>
        <Text
          allowFontScaling={false}
          style={{fontSize:22,color:'#2A2A2A',fontWeight:'bold'}}
          testID={testID ? `${HEADER_MONTH_NAME}-${testID}` : HEADER_MONTH_NAME}
          {...webProps}
        >
          {dateString.split(' ')[0]}
        </Text>
        <Text style={{fontSize:16,color:'#2A2A2A',margin:5}}>年</Text>
        <Text style={{fontSize:22,color:'#2A2A2A',fontWeight:'bold'}}>{dateString.split(' ')[1]}</Text>
        <Text style={{fontSize:16,color:'#2A2A2A',margin:5}}>月</Text>
      </Fragment>
    );
  };
