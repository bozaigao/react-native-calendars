...
import LinearGradientView from '../custom/LinearGradientView';
...
export default class BasicDay extends Component {
  ...
  renderContainer() {
    const {activeOpacity,selected} = this.marking;
    ...
    return (
      ...
      >
        {
          selected?
          <View style={{justifyContent:'center',alignItems:'center',width:34,height:34}}>
           <LinearGradientView style={{width:34,height:34,borderRadius:17,justifyContent:'center',alignItems:'center'}}>
            {this.isMultiPeriod() ? this.renderText() : 
        this.renderContent()
        }
          </LinearGradientView>
          </View>
          :
          <View style={{justifyContent:'center',alignItems:'center',width:34,height:34,backgroundColor:this.isToday()?'#F0F9F4':'white',borderRadius:17}}>
          {this.isMultiPeriod() ? this.renderText() : 
          this.renderContent()
      }
        </View>
